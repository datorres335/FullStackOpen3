...
describe('Blog app', () => {
  ...
  describe('Login', () => {
    ...
    test('fails with wrong credentials', async ({ page }) => {
      ...
      await expect(page.getByText('David logged in')).not.toBeVisible()
    })
  })

  describe('When logged in', () => {
    beforeEach(async ({ page }) => {
      await login(page, 'David', 'password')
    })

    test('a new blog can be created', async ({ page }) => {
      await page.getByRole('button', { name: 'create new blog' }).click()
      await page.getByTestId('author').fill('Ted Tester')
      await page.getByTestId('url').fill('www.url.example')
      await page.getByRole('button', { name: 'Create' }).click()
    })

    describe('and a blog exists', () => {
      beforeEach(async ({ page }) => {
        await createBlog(page, 'Testing with Playwright', 'Ted Tester', 'www.url.example')
      })
